...
async def insert_metaval(db, blob_store, sample_id=None, taxon_name="Shigella-virus-Moo19",
                          classifier="kraken2", with_reads=True, with_igv=True):
    case_oid   = ObjectId()
    sample_oid = ObjectId(sample_id) if sample_id else ObjectId()

    reads_keys = {}
    if with_reads:
        read_key = f"verification_data/{case_oid}/SRR001/{classifier}/{taxon_name}_read_1.fa"
...
class TestGetMetaval:
    ...
    async def test_verification_data_exposes_availability(self, client, fake_db, fake_blob):
