...
class TestExtractClassifierQcKraken2:
    ...
    def _qc_data(self, col="SAMPLE1_k2_pluspf.kraken2.kraken2.report", records=None):
        ...
        return {"kraken2": {key: make_kraken2_records() if records is None else records}}

    def test_happy_path_returns_expected_fields(self):
        result = _extract_classifier_qc(self._qc_data(), "kraken2", "SAMPLE1_k2_pluspf.kraken2.kraken2.report")
        assert set(result.keys()) == {
            "pct_unclassified", "unclassified_reads", "classified_reads",
            "total_reads", "num_species", "num_genera",
        }

    def test_pct_unclassified_correct(self):
        records = make_kraken2_records(unclassified=200, classified=800)
        result = _extract_classifier_qc(self._qc_data(records=records), "kraken2", "SAMPLE1_k2_pluspf.kraken2.kraken2.report")
    ...
    def test_all_zero_counts_become_none(self):
        records = make_kraken2_records(unclassified=0, classified=0, num_species=0, num_genera=0)
        result = _extract_classifier_qc(self._qc_data(records=records), "kraken2", "SAMPLE1_k2_pluspf.kraken2.kraken2.report")
    ...
    def test_missing_key_returns_empty_dict(self):
        result = _extract_classifier_qc({"kraken2": {}}, "kraken2", "SAMPLE1_k2_pluspf.kraken2.kraken2.report")
        assert result == {}

    def test_empty_records_returns_empty_dict(self):
        result = _extract_classifier_qc(self._qc_data(records=[]), "kraken2", "SAMPLE1_k2_pluspf.kraken2.kraken2.report")
        assert result == {}
...
class TestExtractClassifierQcCentrifuge:
    ...
    def test_happy_path_returns_expected_fields(self):
        result = _extract_classifier_qc(self._qc_data(), "centrifuge", "SAMPLE1_p_compressed+h+v.centrifuge")
    ...
    def test_no_root_record_falls_back_to_species_sum(self):
        ...
        result = _extract_classifier_qc(self._qc_data(records=records), "centrifuge", "SAMPLE1_p_compressed+h+v.centrifuge")
...
class TestExtractBaseQcFastp:
    ...
    def _qc_data(self, sample_id="SAMPLE1", lane_suffix="_1", **kwargs):
        return {"fastp": {f"{sample_id}{lane_suffix}": make_fastp_lane(**kwargs)}, "bowtie2": {}, "fastqc": {}}
    ...
    def test_happy_path_fields_present(self):
        ...
        assert set(result["fastp"].keys()) == {
            "total_reads_before_filtering", "total_reads_after_filtering",
            "passed_filter_reads", "low_quality_reads", "too_short_reads",
...
class TestExtractBaseQcBowtie2:
    ...
    def _qc_data(self, sample_id="SAMPLE1", lane_suffix="_1", **kwargs):
        return {"fastp": {}, "bowtie2": {f"{sample_id}{lane_suffix}": make_bowtie2_lane(**kwargs)}, "fastqc": {}}
...
class TestExtractBaseQcFastqc:
    ...
    def _qc_data(self, sample_id="SAMPLE1"):
        return {
            ...
            "bowtie2": {},
            "fastqc": {
                f"{sample_id}_1_raw_1": {"total_sequences": 500000, "avg_sequence_length": 150.0, "percent_gc": 48.0, "percent_fails": 2.0},
                f"{sample_id}_1_raw_2": {"total_sequences": 500000, "avg_sequence_length": 150.0, "percent_gc": 50.0, "percent_fails": 3.0},
            },
        }
